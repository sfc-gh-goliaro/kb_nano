...
class KernelBenchResult:
    ...
    def print_table(self, single_target: bool = False) -> None:
        ...
        for op in self.operators:
            ...
            print(f"{'=' * 70}")
            print()
            print(f"  {'SCENARIO':<40} {'CORRECT':>8}   {'MEAN_DIFF':>10}   {'SPEEDUP':>8}")
            print(f"  {'─' * 66}")
            for s in op.scenarios:
                status = "PASS" if s.correct else "FAIL"
                print(
                    f"  {s.name:<40} {status:>8}   {s.mean_abs_diff:>10.1e}   {s.speedup:>7.2f}x"
                )
            print(f"  {'─' * 66}")
        ...
        if not single_target and len(self.operators) > 1:
            ...
            print(
                ...
                f"{'AVG MEAN_DIFF':>13}   {'AVG SPEEDUP':>11}   {'STATUS':>6}"
            )
            print(f"  {'─' * 68}")
            for op in self.operators:
            ...
            print(
                ...
                f"overall avg mean_diff: {self.avg_mean_abs_diff:.1e}   "
