"""Semantic PyTorch reference for video_processor.
...
VAE output post-processor for images and video: denormalize from [-1, 1] to
[0, 1], permute to HWC, convert to numpy/PIL.

``postprocess``: 4D ``(B, C, H, W)`` image tensors.
``postprocess_video``: 5D ``(B, C, T, H, W)`` video tensors.

All arithmetic is torch/numpy.  ``PIL`` is imported inside the
``output_type == "pil"`` branch: it performs no computation, it only wraps the
already-final uint8 array in an image container, so the reference stays
importable with torch alone and every numeric path is library-free.
"""
