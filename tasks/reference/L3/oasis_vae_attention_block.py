"""Oasis VAE attention block (L3 composite).

Naive pure-torch reference for ``tasks/baseline/L3/oasis_vae_attention_block.py``:
pre-norm residual block wiring OasisVAEAttention (2D axial rotary + dense
self-attention over the flattened frame) and OasisMLP (exact GELU).
"""
