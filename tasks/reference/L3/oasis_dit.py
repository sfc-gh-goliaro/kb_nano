"""Oasis diffusion transformer (L3 composite).

Naive pure-torch reference for ``tasks/baseline/L3/oasis_dit.py``: patch
embedding, timestep conditioning (+ optional external condition), a stack of
spatio-temporal DiT blocks with axial rotary attention, and the final
adaLN-modulated projection + unpatchify.
"""
...
# Inlined from tasks/baseline/L3/oasis_block.py (naive re-implementation)
