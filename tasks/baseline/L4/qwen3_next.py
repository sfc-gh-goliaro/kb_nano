...
class Qwen3NextModel(nn.Module):
    ...
    def forward(self, input_ids, positions, layer_states=None):
        ...
        r2d = residual.reshape(-1, shape[-1]) if residual is not None else None
        if r2d is not None:
            hidden_states, _ = self.norm(h2d, r2d)
        else:
            hidden_states = self.norm(h2d)
        return hidden_states.reshape(shape[:-1] + (hidden_states.shape[-1],))
...
class Qwen3NextForCausalLM(nn.Module):
    ...
    def forward(self, input_ids, positions, layer_states=None):
        return self.model(input_ids, positions, layer_states=layer_states)
