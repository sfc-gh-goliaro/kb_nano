...
from ._fa_backend import fa_varlen_opaque as _fa_varlen_opaque
from ._fa_backend import fa_version_for_head_dim as _fa_version_for_head_dim
from ._fa_backend import use_opaque_fa as _use_opaque_fa
...
class FlashAttnVarlen(nn.Module):
    ...
    def forward(
        ...
        if _use_opaque_fa(max_seqlen_k) and not return_softmax_lse:
            # Blackwell: FA4 is not dynamo-traceable, but callers of this op run
            # inside torch.compile, so the untraceable-backend rule would drop us
            # to upstream FA2. Route through the custom op instead, which is
            # opaque to dynamo. Only above the length threshold -- FA4's per-call
            # dispatch cost makes it a large net loss on short sequences (see
            # _fa_backend.FA_OPAQUE_MIN_SEQ). LSE is not plumbed through the op,
            # so that variant still takes the FA2 path below.
            version = _fa_version_for_head_dim(q.shape[-1])
            if version is not None:
                v_padded = _maybe_pad_v(q, v, version)
                return _unpad_out(
                    _fa_varlen_opaque(
                        q, k, v_padded, cu_seqlens_q, cu_seqlens_k,
