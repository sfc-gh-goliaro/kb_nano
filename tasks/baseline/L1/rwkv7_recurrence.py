...
state update:
    S_t = diag(exp(w_t)) @ S_{t-1} + (kk_t * a_t) @ ((-kk_t)^T @ S_{t-1}) + k_t @ v_t^T
    o_t = r_t^T @ S_t
...
def naive_recurrent_rwkv7(
    ...
    for i in range(T):
        r_i = r[:, :, i].float() * scale           # [B, H, K]
        w_i = w[:, :, i].float()                    # [B, H, K]
        k_i = k[:, :, i].float()                    # [B, H, K]
        v_i = v[:, :, i].float()                    # [B, H, V]
        kk_i = kk[:, :, i].float()                  # [B, H, K]
        a_i = a[:, :, i].float()                    # [B, H, K]
        ...
        correction = (act_a[..., None] * h).sum(-2)                     # [B, H, V]
        h = w_i.exp()[..., None] * h + bb[..., None] * correction[..., None, :]
        # Outer product update
        h = h + k_i[..., None] * v_i[..., None, :]                     # [B, H, K, V]
