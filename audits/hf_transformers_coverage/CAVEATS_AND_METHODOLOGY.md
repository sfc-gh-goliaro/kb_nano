...
- `esmfold` — EsmFoldTriangleAttention is AlphaFold-2-era triangular attention (LayerNorm + EsmFoldLinear + EsmFoldAttention with bias support + chunk_layer / use_memory_efficient_kernel flags). kb-nano has `L2/alphafold3_triangle_attention.py:TriangleAttention` with the same `(c_in, c_hidden, no_heads, starting, inf)` signature, but the kb-nano variant references AF3's OF3Attention (openfold3 lineage) — interop with the AF2-era ESMFold variant is not verified. EsmFoldTriangleMultiplicativeUpdate has a sibling `L2/alphafold3_triangle_multiplication.py` but again AF3-keyed. Partial because the AF2↔AF3 mapping isn't established by the audit.
