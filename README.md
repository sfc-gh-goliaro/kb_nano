...
Throughput (Jamba-tiny-dev, 50 sequences per scenario, max-output=128, max-prompt=1024, `temperature=0`, kb-nano `max_num_seqs=32`, vLLM `max_num_seqs=32`, eager mode -- `KB_NANO_JAMBA_COMPILE=0`):

| Model | TP | Scenario | Input/Output | vLLM (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-------------:|-------------:|------:|-----------------:|
| Jamba-tiny-dev | 1 | prefill-heavy | 1024/128 | 6,932 | 11,348 | **1.64x** |  94.8/128 |
| Jamba-tiny-dev | 1 | balanced      |  512/128 | 8,151 | 12,012 | **1.47x** |  85.0/128 |
| Jamba-tiny-dev | 1 | decode-heavy  |  512/128 | 8,420 | 12,185 | **1.45x** |  91.1/128 |

Geomean ratio: **1.52×** vs vLLM 0.18.0.  Avg match-tokens **90.3/128** (above the ≥50/128 bar).  With `KB_NANO_JAMBA_COMPILE=1` the geomean speedup rises to ~1.85× but match-tokens collapse to ~11/128 (Inductor's bf16 reduction order diverges from vLLM's CUDA kernels) -- correctness wins by default.
...
**Jamba-v0.1 (52B), kb-nano vs vLLM 0.18.0 reference (50 sequences per scenario, max-output=128, max-prompt=1024, `temperature=0`, kb-nano `max_num_seqs=32`, vLLM `max_num_seqs=32`, eager mode):**

| Model | TP | Scenario | Input/Output | vLLM (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-------------:|-------------:|------:|-----------------:|
| Jamba-v0.1 | 1 | prefill-heavy | 1024/128 | 823 | 856 | **1.04x** | 90.1/128 |
| Jamba-v0.1 | 1 | balanced      |  512/128 | 901 | 938 | **1.04x** | 78.0/128 |
| Jamba-v0.1 | 1 | decode-heavy  |  512/128 | 924 | 893 |  0.97x  | 89.7/128 |

Geomean ratio at 52B: **1.02×** vs vLLM (above the ≥0.9× ship bar with margin).  Avg match-tokens **85.9/128** (above the ≥50/128 bar).  Reproducible across runs.  vLLM throughput shows ~10-15% per-scenario noise across runs depending on which other GPUs share the B200; kb-nano's absolute throughput is stable around 850-940 tok/s.

Five structural changes drove the work to this state:
...
3. **Continuous batching with chunked prefill mid-decode.**  `Sequence` + `BlockManager` + `_MambaSlotPool` scheduler with `waiting / running` deques and three step types (mixed prefill+decode / pure prefill / pure decode); workspace sharing across `Attention` modules (`set_trtllm_workspace`, mirroring `LlamaEngine._share_trtllm_workspace`) reclaims ~3.5 GB on Blackwell.  The mixed-step path mirrors `_generate_mamba.run_mamba_mixed`'s prefill-first concatenated-batch convention (verified deterministic on var-`max_tokens` workloads where the mixed path actually fires; on the uniform-`max_tokens` bench above the path is structurally available but the slot-pool constraint prevents trigger).  The big v0.1 perf jump (0.90× → 1.02×) over the prior lockstep engine is from no longer padding the trailing 18-prompt prefill batch up to `max_num_seqs=32` with prompt clones -- that ~14×1024 = 14336 wasted prefill tokens / batch is reclaimed.
