...
**Continuous-batching scheduler with within-seq chunked prefill.**  Sequences live as `Sequence` objects (token_ids, generated_ids, block_table, num_computed_tokens, state_slot, status -- field-compatible with `infra.engine.Sequence`).  `BlockManager` allocates paged-KV blocks per seq from a free-block deque; `_MambaSlotPool` allocates Mamba slots from a free-slot deque.  The scheduler maintains three deques (`waiting` / `prefilling` / `running`) and runs **two phase-pure step types** per iteration -- the same FLA-engine pattern (`infra/fla_engine.py`):
  ...
  2. **Pure decode step** over `running` -- the captured CUDA-graph fast path.  Dispatches to the smallest bucket ≥ live running batch size (buckets `[1, 2, 4, 8, 16, 24, 32, 40, …, max_num_seqs]`, default max_num_seqs=256 to match the rest of the project's LLM benches), padding tail rows with the project-standard sentinels (`slot_mapping=-1`, `context_lens=0`, `cache_indices=-1`) so the captured graph runs cleanly on partial buckets without race-corrupting the real seqs' Mamba state.  GPU greedy argmax + async D2H pipelines next-token IDs to a pinned host buffer (mirrors `run_mamba_decode_fast_async` / `_wait_async_mamba_tokens`).
...
Geomean ratio: **2.40×** vs vLLM 0.18.0.  Avg match-tokens (tokens where kb-nano and vLLM agree exactly): **532/748** = 71.1% across the three scenarios -- well above the ≥100 match-tokens/req correctness bar.  With `KB_NANO_JAMBA_COMPILE=1` geomean rises further but match-tokens collapse (Inductor's bf16 reduction order diverges from vLLM's CUDA kernels) -- correctness wins by default.

Latency (single-request and fixed-batch-32, 128 output tokens, 5 iterations):
...
**Jamba-v0.1 (52B), kb-nano vs vLLM 0.18.0 reference (1000 sequences per scenario, max-prompt=1024, max-output=1024 cap, `temperature=0`, `max_num_seqs=256`, eager mode):**
...
Geomean ratio at 52B: **1.02×** vs vLLM (above the ≥0.9× ship bar with margin).  Avg match-tokens (kb-nano vs vLLM, exact-prefix-match): **415/748** = 55.5%, comfortably above the ≥100 match-tokens/req correctness bar in absolute count.  Reproducible across runs.
