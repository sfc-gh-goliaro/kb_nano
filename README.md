...
- **Jamba** (AI21Labs `Jamba-tiny-dev`, `Jamba-v0.1`) triple-hybrid Transformer + Mamba-1 + sparse MoE: per-block layer schedule (1 attention every 8 layers, 1 sparse-MoE FFN every 2 layers), per-layer dt/B/C RMSNorms on the SSM mixer, top-k softmax MoE routing without renormalisation, and a dedicated single-rank `JambaEngine` that owns both a transformer KV slab and a per-sequence Mamba selective-scan state. The model interface follows the project's standard convention: `forward(input_ids, positions)` -> hidden states; per-step KV / Mamba state is published on the global `Context` via `set_jamba_context` and consumed by L2 mixers (the same pattern Llama uses with `set_context` and Mamba uses with `set_mamba_context`). The decode loop runs through a `torch.compile`-wrapped forward + a single captured CUDA graph (Inductor fuses the per-step elementwise tail — RMSNorm chains, residual adds, SwiGLU gate*up, MoE softmax pieces — into Triton kernels, eliminating ~95% of the small elementwise launch overhead; CUDA graph then records the compiled kernels for replay-based decode). Logit-level correctness: kb-nano's logits agree with HF within HF's own cross-batch bf16 noise floor. Greedy-token-match on real wildchat data: ≈95/128 on tiny-dev — well above the bench's ≥50/128 correctness bar. **Geomean 1.52× tok/s vs vLLM 0.18.0 on tiny-dev and 0.82× on Jamba-v0.1 (52B)** (across prefill-heavy / balanced / decode-heavy scenarios; tiny-dev N=200, v0.1 N=50). The 52B gap is structural: kb-nano uses dense KV slabs vs vLLM's paged KV + continuous batching scheduler; kernel time has been reduced as far as `torch.compile` can take it (post-compile profile: elementwise = 3.4% of GPU time, down from 41.8%).
...
| Model | TP | Scenario | Input/Output | vLLM (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-------------:|-------------:|------:|-----------------:|
| Jamba-tiny-dev | 1 | prefill-heavy | 1024/128 |  8,469 | 12,389 | **1.46x** |  99.3/128 |
| Jamba-tiny-dev | 1 | balanced      |  512/128 |  8,073 | 14,780 | **1.83x** |  89.2/128 |
| Jamba-tiny-dev | 1 | decode-heavy  |  512/128 |  9,152 | 12,098 | **1.32x** |  95.5/128 |

Geomean ratio across the three scenarios: **1.52×** vs vLLM. Token alignment averages **94.7/128**, well above the bench's correctness bar (≥50/128). Driven primarily by the `torch.compile`-fused decode forward inside the captured CUDA graph: the per-step dispatcher overhead from ~110k tiny elementwise kernel launches is now reduced to a few dozen Inductor-fused Triton kernels.
...
| Model | TP | Scenario | Input/Output | vLLM (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-------------:|-------------:|------:|-----------------:|
| Jamba-v0.1 | 1 | prefill-heavy | 1024/128 | 812 | 712 | 0.88x |  94.4/128 |
| Jamba-v0.1 | 1 | balanced      |  512/128 | 898 | 732 | 0.82x |  89.0/128 |
| Jamba-v0.1 | 1 | decode-heavy  |  512/128 | 920 | 717 | 0.78x |  84.6/128 |

Geomean ratio at 52B: **0.82×** vs vLLM (improved from 0.68× pre-compile via the `torch.compile`-fused decode forward — see the elementwise kernel-time reduction noted above). Token alignment averages **89.3/128** — well above the bench's correctness bar (≥50/128). vLLM throughput in this environment shows ~10–20% run-to-run variance (a separate post-revert run of the same code produced 1.00× geomean against an unfavourable-warmup vLLM); kb-nano's own throughput is stable at ~715 tok/s across runs.

**The remaining 8–18% gap at 52B is structural, not addressable by `torch.compile`:** kb-nano's `JambaEngine` uses dense left-padded KV slabs (`[B, num_kv_heads, max_total, head_dim]`) instead of vLLM's paged KV + continuous-batching scheduler. With `max_num_seqs=32` and N=50 the second batch carries 14 padding rows (50/64 = 78% slot utilisation); at 52B, the wasted compute on those unused slots compounds with the wasted attention compute on the unused suffix of each row. Profile-driven attribution after `torch.compile`: 27.6% mamba_ssm (vendor kernel, can't be optimised), 23.9% MoE (Triton fused, parity with vLLM), 20.8% GEMM, 10.0% attention, 3.4% elementwise. Closing the residual gap requires re-architecting the attention path onto the project's standard paged-Attention layout (`set_context` with `slot_mapping` / `block_tables`) and adding a continuous-batching scheduler — separate, larger pieces of work.
