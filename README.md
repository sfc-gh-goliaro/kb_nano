...
Run `tests/bench_jamba.py --model ai21labs/Jamba-tiny-dev --ref vllm` to reproduce. The reference is **vLLM 0.18.0**'s production Jamba server (continuous batching, paged KV, FlashInfer attention, FULL+PIECEWISE CUDA-graph capture). On this environment both sides dispatch into the same `causal_conv1d_fn` / `causal_conv1d_update` / `selective_scan_fn` / `selective_state_update` CUDA primitives. The benchmark is a fair pipeline-vs-pipeline comparison at the SOTA bar.

**The L2 modules are vLLM-aligned** so generated kernels port back into vLLM with no call-site change -- which is the whole point of this benchmark.  `JambaAttention` is a structural copy of `LlamaAttention`: fused `QKVParallelLinear` (one matmul instead of three separate `q_proj` / `k_proj` / `v_proj`), the project's `L2.attention_impl.Attention` class (paged-KV store + dispatch to TRTLLM-gen on Blackwell or FA3 elsewhere), `RowParallelLinear` `o_proj`.  `JambaMLP` is identical to `LlamaMLP` -- vLLM aliases the two as one class -- with fused `MergedColumnParallelLinear` `gate_up_proj` + `SiluAndMul` + `RowParallelLinear`.  The L3 decoder layers use Llama-style **fused add+RMSNorm** (`_C.fused_add_rms_norm`) for the residual stream, so the bf16 reduction order matches vLLM's exactly (the prior HF-style separate `h0 + norm(h)` produced ~1e-3 per-layer drift that compounded over 32 layers and tanked match-tokens).

**Engine layout** (`infra/jamba_engine.py`).  One global paged KV cache for the 4 attention layers: `[2, num_attn_layers, num_blocks, page_size, num_kv_heads, head_dim]` (NHD on Hopper) or `[2, num_attn_layers, num_blocks, num_kv_heads, page_size, head_dim]` (HND on Blackwell, page_size=16 for TRTLLM-gen / 256 for FA3) -- same allocation pattern as `LlamaEngine.allocate_kv_cache`.  Each `Attention` module gets bound to its slice via `module.k_cache = kv_cache[0, i]` / `module.v_cache = kv_cache[1, i]`.  Per-seq Mamba `(conv_state, ssm_state)` slot pool for the 28 Mamba layers, allocated/released per request through a `_MambaSlotPool` (single-rank counterpart of `MambaStateManager`).  Pipeline is flat-varlen `[N, hidden]` throughout: prefill packs admitted prompts into one `[total_real_tokens, hidden]` tensor with `cu_seqlens_q`, decode runs `[B_running, hidden]` per step.  Per-step state flows through the project's standard `set_context` fields (`slot_mapping` / `block_tables` / `context_lens` / `cu_seqlens` plus `mamba_state` / `mamba_metadata`) installed by a single `set_jamba_context` helper.  Workspace sharing across `Attention` modules (`set_trtllm_workspace` mirroring `LlamaEngine._share_trtllm_workspace`) reclaims ~3.5 GB that per-layer-private workspaces would otherwise burn on Blackwell (4 attn layers × 2 ops × 512 MB each).

**Continuous-batching scheduler with within-seq chunked prefill.**  Sequences live as `Sequence` objects (token_ids, generated_ids, block_table, num_computed_tokens, state_slot, status -- field-compatible with `infra.engine.Sequence`).  `BlockManager` allocates paged-KV blocks per seq from a free-block deque; `_MambaSlotPool` allocates Mamba slots from a free-slot deque.  The scheduler maintains three deques (`waiting` / `prefilling` / `running`) and runs **two phase-pure step types** per iteration -- the same FLA-engine pattern (`infra/fla_engine.py`):

  1. **Chunked prefill step** over the `prefilling` deque.  `_next_prefill_chunks` selects this step's `(seq, chunk_size)` pairs capped at `max_num_batched_tokens` (default 16384, override per-engine).  Each chunk contributes `chunk_size` consecutive prompt tokens; `has_initial_state[i] = (num_computed_i > 0)` tells both the conv and SSM kernels to continue from the prior chunk's `(conv_state, ssm_state)` slab at `cache_indices[i]` rather than initialise from zero.  Paged attention sees the full `cu_seqlens_k` (prior-chunk K/V already in cache + this chunk's K/V about to be written via `slot_mapping`); Q is positioned at the end of K via causal masking, exactly as TRTLLM-gen / FA3 prefix-cache prefill expects.  Seqs whose prefill completes this step (last chunk) sample their first decode token and join `running`; mid-prefill seqs stay in `prefilling`.  Mirrors `LlamaEngine.prepare_mixed_batch` / `_generate_mamba._mamba_prepare_tensors`'s per-seq `start = num_computed_tokens` chunk slicing and `FLAEngine._next_prefill_batch`'s deque-driven scheduler.
  2. **Pure decode step** over `running` -- the captured CUDA-graph fast path.  Dispatches to the smallest bucket ≥ live running batch size (buckets `[1, 2, 4, 8, 16, 24, 32]` for `max_num_seqs=32`), padding tail rows with the project-standard sentinels (`slot_mapping=-1`, `context_lens=0`, `cache_indices=-1`) so the captured graph runs cleanly on partial buckets without race-corrupting the real seqs' Mamba state.  GPU greedy argmax + async D2H pipelines next-token IDs to a pinned host buffer (mirrors `run_mamba_decode_fast_async` / `_wait_async_mamba_tokens`).

This is the same pattern as `FLAEngine.generate` -- waiting / prefilling / running deques, phase-pure prefill and decode steps, eager-allocated lifetime blocks at admit time so decode-step block_tables don't grow.  The `Sequence` and `BlockManager` definitions are local to `jamba_engine.py` because importing `infra.engine` transitively pulls in the vision / MLA / encoder-decoder model zoo and JambaEngine doesn't need any of it; `FLAEngine` handles the same problem the same way (its local `_ActiveSeq`).  The L2 `JambaMambaMixer` and L2 `Attention` both retain their native mixed-batch (prefill+decode in one forward) branches as part of the standard kb-nano L2 API for portability into vLLM, even though the JambaEngine's phase-pure scheduler doesn't fire them.

**`torch.compile` is OFF by default for Jamba** (set `KB_NANO_JAMBA_COMPILE=1` to enable).  We tried it: Inductor fuses the per-layer elementwise tail (RMSNorm + residual + SwiGLU pieces) into compact Triton kernels and gives ~25-30% extra throughput on tiny-dev (1.48× → 1.85× geomean) and a small lift on v0.1 (~0.90× → ~0.93×).  But its **fused Triton kernels produce different bf16 intermediate values from vLLM's hand-written `_C.fused_add_rms_norm` / `_C.silu_and_mul` kernels** -- ~1e-3 drift per layer that, over 32 layers and 128 decode steps, flips greedy top-1 within ~5 tokens for ~80% of prompts and tanks the bench's match-tokens-vs-vLLM metric from ~85/128 down to ~25/128.  Inspecting raw bench outputs makes this obvious: with compile on, the divergence pattern is bimodal -- 5-8 prompts perfect-match, ~30 prompts diverge in first 1-5 tokens.  With compile off (eager), the kb-nano forward calls the *same* CUDA kernels vLLM does, so bf16 is byte-identical and the divergence reverts to the slow accumulation pattern that lets ~half the prompts match through 100+ steps.  Match-tokens correctness matters more than 25% extra speed at the 0.9× perf bar, so eager wins by default.  (When compile is on, we still capture a CUDA graph around the compiled forward; CUDA graph alone -- without compile -- is what carries the eager path's speed.)

Per-step KV and Mamba state flow through the project's standard global-Context pattern: the engine wraps every forward in `set_jamba_context(...)` / `reset_context()`, which populates the **standard** `Context` fields (`is_prefill`, `slot_mapping`, `block_tables`, `context_lens`, `max_context_len`, `cu_seqlens_q/k`, `max_seqlen_q/k`, `flat_to_grid`, `mamba_state`, `mamba_metadata`) — the same fields `set_context` / `set_mamba_context` populate for Llama / Mamba / Mamba2 / Mixtral, just installed in one helper because Jamba's hybrid forward needs both attention and Mamba metadata in a single Context. The L2 mixers read their per-layer paged cache off `self.k_cache` / `self.v_cache` (bound by the engine after KV allocation, exactly like `LlamaAttention`) and read the per-step plumbing off `get_context()`. This keeps the model's forward signature aligned with Llama / Mamba / Mamba2 / Mixtral (`model(input_ids, positions)` -> hidden states), so the L4 / L3 / L2 module hierarchy matches the rest of the codebase.

Throughput (Jamba-tiny-dev, 200 sequences per scenario, max-prompt=1024, max-output=1024 cap, `temperature=0`, kb-nano `max_num_seqs=32`, vLLM `max_num_seqs=32`, eager mode -- `KB_NANO_JAMBA_COMPILE=0`).  WildChat-derived datasets provide the actual prefill / decode mix per scenario (`prefill-heavy` ≈ 1024/512, `balanced` ≈ 512/512, `decode-heavy` ≈ 512/1024 input/output):

| Model | TP | Scenario | Input/Output | vLLM (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-------------:|-------------:|------:|-----------------:|
| Jamba-tiny-dev | 1 | prefill-heavy | 1024/610  |  8,194 | 12,559 | **1.53x** | 461.4/610  |
| Jamba-tiny-dev | 1 | balanced      |  512/636  |  8,299 | 12,796 | **1.54x** | 365.9/636  |
| Jamba-tiny-dev | 1 | decode-heavy  |  512/1006 | 10,196 | 15,333 | **1.50x** | 691.7/1006 |

Geomean ratio: **1.52×** vs vLLM 0.18.0.  Avg match-tokens (tokens where kb-nano and vLLM agree exactly): **506/751** = 67.4% across the three scenarios -- well above the ≥50/128 mentor bar.  With `KB_NANO_JAMBA_COMPILE=1` the geomean speedup rises to ~1.85× but match-tokens collapse (Inductor's bf16 reduction order diverges from vLLM's CUDA kernels) -- correctness wins by default.

Latency (single-request and fixed-batch-32, 128 output tokens, 3 iterations):

| Model | TP | Scenario | Batch Size | vLLM median | Ours median | vLLM ms/tok | Ours ms/tok | Ratio |
|-------|---:|----------|---:|------------:|------------:|------------:|------------:|------:|
| Jamba-tiny-dev | 1 | single-request | 1  | 0.187s | 0.177s | 1.46 | 1.38 | **1.06x** |
| Jamba-tiny-dev | 1 | fixed-batch-32 | 32 | 0.412s | 0.289s | 0.10 | 0.07 | **1.43x** |
...
**Jamba-v0.1 (52B), kb-nano vs vLLM 0.18.0 reference (50 sequences per scenario, max-prompt=1024, max-output=1024 cap, `temperature=0`, kb-nano `max_num_seqs=32`, vLLM `max_num_seqs=32`, eager mode):**

| Model | TP | Scenario | Input/Output | vLLM (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-------------:|-------------:|------:|-----------------:|
| Jamba-v0.1 | 1 | prefill-heavy | 1024/604  | 1,191 | 1,231 | **1.03x** | 375.2/604  |
| Jamba-v0.1 | 1 | balanced      |  512/647  | 1,233 | 1,196 |  0.97x  | 331.1/647  |
| Jamba-v0.1 | 1 | decode-heavy  |  512/1007 | 1,349 | 1,330 |  0.99x  | 562.9/1007 |

Geomean ratio at 52B: **1.00×** vs vLLM (within noise of parity, above the ≥0.9× ship bar with margin).  Avg match-tokens (kb-nano vs vLLM, exact-prefix-match): **423/753** = 56.2%, comfortably above the ≥50-token correctness bar in absolute count.  Reproducible across runs.  vLLM throughput shows ~5-15% per-scenario noise across runs depending on which other GPUs share the B200; kb-nano's absolute throughput is stable around 1,196-1,330 tok/s.

Latency (single-request and fixed-batch-32, 128 output tokens, 3 iterations):

| Model | TP | Scenario | Batch Size | vLLM median | Ours median | vLLM ms/tok | Ours ms/tok | Ratio |
|-------|---:|----------|---:|------------:|------------:|------------:|------------:|------:|
| Jamba-v0.1 | 1 | single-request | 1  | 0.922s | 0.939s | 7.20 | 7.33 | 0.98x |
| Jamba-v0.1 | 1 | fixed-batch-32 | 32 | 3.898s | 4.067s | 0.95 | 0.99 | 0.96x |

Six structural pieces drove the work to this state:

1. **vLLM-aligned L2 modules** -- fused `QKVParallelLinear` (one matmul, matches vLLM's reduction order) + the project's `Attention` class (same paged-KV TRTLLM-gen / FA3 dispatch as vLLM) + `RowParallelLinear`; fused `gate_up_proj` + `SiluAndMul` mirroring `LlamaMLP` (which vLLM aliases as `JambaMLP`); Llama-style fused add+RMSNorm in the L3 layers.  L2 modules drop into vLLM's `JambaAttentionDecoderLayer` / `JambaMambaDecoderLayer` with no call-site change -- the portability point of kb-nano.
2. **Paged KV cache + flat-varlen pipeline** through the standard `set_context` fields -- same plumbing as `LlamaEngine`, so the L2 task interface is portable into vLLM with no call-site change.
3. **Continuous batching with within-seq chunked prefill.**  `Sequence` + `BlockManager` + `_MambaSlotPool` scheduler with `waiting / prefilling / running` deques and two phase-pure step types (chunked prefill / pure decode), mirroring `FLAEngine.generate`.  Long prompts get split across multiple prefill steps capped at `max_num_batched_tokens`; the L2 Mamba mixer's `has_initial_state` flag carries `(conv_state, ssm_state)` continuation across chunks, and paged-attn sees the full `cu_seqlens_k` so each chunk's Q attends to the full prior K/V from the cache.  Workspace sharing across `Attention` modules (`set_trtllm_workspace`, mirroring `LlamaEngine._share_trtllm_workspace`) reclaims ~3.5 GB on Blackwell.
4. **Multi-bucket CUDA graph capture** for pure-decode steps at sizes `[1, 2, 4, 8, 16, 24, 32]` with shared mempool (largest-first capture order, mirroring `LlamaEngine.capture_cudagraph`).  Pad rows in partial buckets use the project-standard sentinels (`slot_mapping=-1`, `context_lens=0`, `cache_indices=-1`) so the captured graph runs cleanly without race-corrupting the real seqs' Mamba state.
5. **Async D2H pipeline for next-token IDs** -- mirrors the Mamba engine's `run_mamba_decode_fast_async` / `_wait_async_mamba_tokens` pattern.  After a captured decode graph replays, GPU `next_tokens[:n]` is copied to a pinned host buffer on a separate stream with an event recorded; the caller waits on that event before reading.  Replaces the prior blocking `torch.cuda.synchronize()` in the per-step hot loop.
6. **Eager mode by default** -- `torch.compile`'s Inductor-fused elementwise kernels drift from vLLM's hand-written `_C.fused_add_rms_norm` / `_C.silu_and_mul` by ~1e-3 per layer in bf16; over 32 layers and 128 decode steps that flips greedy top-1 within ~5 tokens for most prompts.  Disabling compile (and using vLLM's own kernels directly) fixes match-tokens at small perf cost.

**Note on chunked-prefill numerics.**  For real natural-language prompts the chunked path is bit-stable vs one-shot.  On a degenerate synthetic test (1500 random integer IDs, forced 3 chunks at `max_num_batched_tokens=512`), bf16 rounding flips the greedy top-1 because the top-1 / top-2 logits land within ~0.04 of each other (chunked-vs-one-shot mean abs diff ≈ 0.036, max ≈ 0.19; both candidates appear in each top-5 list).  The bench above does NOT trigger chunking (max-prompt=1024 < 16384 default budget), so this is a forward-compatibility caveat for very long prompts, not a parity claim.
