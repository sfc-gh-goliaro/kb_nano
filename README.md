...
- **Jamba** (AI21Labs `Jamba-tiny-dev`, `Jamba-v0.1`) triple-hybrid Transformer + Mamba-1 + sparse MoE.  L2 modules mirror vLLM's exactly so generated kernels port back into vLLM with no call-site change: `JambaAttention` uses fused `QKVParallelLinear` + the project's `L2.attention_impl.Attention` class (same paged-KV / TRTLLM-gen / FA3 dispatch as `LlamaAttention`) + `RowParallelLinear`; `JambaMLP` mirrors `LlamaMLP` (fused `gate_up_proj` + `SiluAndMul` + `RowParallelLinear`); the L3 decoder layers use Llama-style fused add+RMSNorm (`fused_add_rms_norm`) for residuals.  The full pipeline is flat-varlen `[N, hidden]`: prefill packs prompts into one `[total_tokens, hidden]` tensor with `cu_seqlens`, decode is `[B, hidden]`, and per-step paged-KV / Mamba state flows through the project's standard `set_context` fields (`slot_mapping` / `block_tables` / `context_lens` / `cu_seqlens` plus `mamba_state` / `mamba_metadata`) installed by a single `set_jamba_context` helper.  Both prefill and decode go through paged kernels (`TRTLLMPrefill` + `TRTLLMDecode` on Blackwell, `FlashAttnPrefill` + `FlashAttnDecode` elsewhere).  Correctness: **86.7/128 avg match-tokens vs vLLM 0.18.0 on Jamba-v0.1 (52B), 92/128 on tiny-dev** -- well above the ≥50/128 bar.  **Geomean 1.48× tok/s vs vLLM on tiny-dev and 0.90× on Jamba-v0.1** across prefill-heavy / balanced / decode-heavy scenarios (N=50, vLLM's own warmup variance gives ±10% per-scenario noise on a shared B200).  `torch.compile` is OFF by default for Jamba: Inductor's fused elementwise paths (RMSNorm + residual + SwiGLU) drift from vLLM's hand-written CUDA kernels by ~1e-3 per layer in bf16 and tank match-tokens to ~26/128, so we keep the eager path that uses the same `_C.fused_add_rms_norm` / `_C.silu_and_mul` kernels vLLM uses.  Set `KB_NANO_JAMBA_COMPILE=1` to enable compile if you can tolerate that drift in exchange for ~25-30% extra throughput at tiny-dev scale (the v0.1 perf gap from compile is small because GEMMs / Mamba / MoE dominate at 52B).
...
**Engine layout** (`infra/jamba_engine.py`).  One global paged KV cache for the 4 attention layers: `[2, num_attn_layers, num_blocks, page_size, num_kv_heads, head_dim]` (NHD on Hopper) or `[2, num_attn_layers, num_blocks, num_kv_heads, page_size, head_dim]` (HND on Blackwell, page_size=16 for TRTLLM-gen / 256 for FA3) -- same allocation pattern as `LlamaEngine.allocate_kv_cache`.  Each `Attention` module gets bound to its slice via `module.k_cache = kv_cache[0, i]` / `module.v_cache = kv_cache[1, i]`.  Per-row Mamba `(conv_state, ssm_state)` slabs for the 28 Mamba layers (slot indices via `cache_indices = arange(B)`, same convention as `MambaEngine` / `FLAEngine`).  **Pipeline is flat-varlen `[N, hidden]` throughout**: prefill packs all prompts into one `[total_real_tokens, hidden]` tensor with `cu_seqlens_q` for kernel batching, decode runs `[B, hidden]` per step.  All per-step state flows through the project's standard `set_context` fields (`slot_mapping` / `block_tables` / `context_lens` / `cu_seqlens` plus `mamba_state` / `mamba_metadata`) installed by a single `set_jamba_context` helper -- same convention `LlamaEngine` uses for paged attention and `MambaEngine` for Mamba state.  Decode runs through a CUDA-graph cache captured at engine init at B=`max_num_seqs`; the host loop mutates `step_input_ids` / `slot_mapping` / `context_lens` in-place between replays so each step writes the new K/V to the next slot in the per-row block_table and attention only touches `[:context_lens[i]]` slots, eliminating the prior dense-slab path's wasted-tail compute (which touched all `graph_max_total` slots regardless of `cur_len`).
...
| Model | TP | Scenario | Input/Output | vLLM (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-------------:|-------------:|------:|-----------------:|
| Jamba-tiny-dev | 1 | prefill-heavy | 1024/128 | 7,291 | 11,163 | **1.53x** | 100.7/128 |
| Jamba-tiny-dev | 1 | balanced      |  512/128 | 8,572 | 11,872 | **1.38x** |  78.9/128 |
| Jamba-tiny-dev | 1 | decode-heavy  |  512/128 | 7,886 | 12,112 | **1.54x** |  96.3/128 |

Geomean ratio: **1.48×** vs vLLM 0.18.0.  Avg match-tokens **92.0/128** (above CLAUDE.md's ≥50/128 bar).  With `KB_NANO_JAMBA_COMPILE=1` the geomean speedup rises to ~1.85× but match-tokens collapse to ~11/128 (Inductor's bf16 reduction order diverges from vLLM's CUDA kernels, see the compile note above) -- correctness wins by default.
...
| Model | TP | Scenario | Input/Output | vLLM (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-------------:|-------------:|------:|-----------------:|
| Jamba-v0.1 | 1 | prefill-heavy | 1024/128 | 808 | 692 | 0.86x |  89.1/128 |
| Jamba-v0.1 | 1 | balanced      |  512/128 | 892 | 811 | **0.91x** | 82.2/128 |
| Jamba-v0.1 | 1 | decode-heavy  |  512/128 | 916 | 858 | **0.94x** | 88.7/128 |

Geomean ratio at 52B: **0.90×** vs vLLM (at the ≥0.9× ship bar).  Avg match-tokens **86.7/128** (above the ≥50/128 bar).  vLLM throughput shows ~10-15% per-scenario noise across runs depending on which other GPUs share the B200; kb-nano's absolute throughput is stable around 690-860 tok/s.

Three structural changes drove the work to this state:
...
3. **Eager mode by default** -- `torch.compile`'s Inductor-fused elementwise kernels drift from vLLM's hand-written `_C.fused_add_rms_norm` / `_C.silu_and_mul` by ~1e-3 per layer in bf16; over 32 layers and 128 decode steps that flips greedy top-1 within ~5 tokens for most prompts.  Disabling compile (and using vLLM's own kernels directly) fixes match-tokens at modest perf cost on v0.1 (0.93× → 0.90×).

What didn't ship in this iteration: continuous batching (admit new sequences mid-decode -- vLLM's remaining ~10% v0.1 win comes from overlapping trailing micro-batch prefill with lead-batch decode).  Adding a `Sequence` + `BlockManager` scheduler to `JambaEngine` is the next structural piece.
