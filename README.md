...
**Continuous-batching scheduler with chunked-prefill mid-decode.**  Sequences live as `Sequence` objects (token_ids, generated_ids, block_table, num_computed_tokens, state_slot, status -- field-compatible with `infra.engine.Sequence`).  `BlockManager` allocates paged-KV blocks per seq from a free-block deque; `_MambaSlotPool` allocates Mamba slots from a free-slot deque.  The scheduler maintains `waiting / running` deques and runs one of three step types per iteration:

  1. **Mixed step** (chunked-prefill mid-decode): when `_admit()` returns new prefill seqs AND `running` is non-empty (a previous running seq finished early via `eos` / `max_tokens` and freed a slot), we run ONE forward pass with the prefill-first concatenated batch.  The L2 `Attention` class already supports this via `Attention._forward_mixed` (which reads `ctx.is_mixed`, `ctx.num_prefill_tokens`, `ctx.prefill_cu_seqlens_q`, `ctx.decode_context_lens`, etc.); the L2 `JambaMambaMixer` has a native mixed-path branch that splits at `num_prefill_tokens` and runs `causal_conv1d_fn` + `selective_scan_fn` on the prefill range and `causal_conv1d_update` + `selective_state_update` on the decode range -- mirrors `infra.engine.run_mamba_mixed`.  Eager only (the mixed-batch shape varies per call).
  2. **Pure prefill step** (only new seqs admitted, no running survivors): batched flat-varlen prefill over the admitted seqs.
  3. **Pure decode step** (only running, no new admits): the captured CUDA-graph fast path -- dispatches to the smallest bucket ≥ live running batch size (buckets `[1, 2, 4, 8, 16, 24, 32]` for `max_num_seqs=32`), padding tail rows with the project-standard sentinels (`slot_mapping=-1`, `context_lens=0`, `cache_indices=-1`) so the captured graph runs cleanly on partial buckets.

This is the same pattern as `LlamaEngine.generate` / `LlamaEngine._generate_mamba` -- pattern-level consistency.  The `Sequence` and `BlockManager` definitions are local to `jamba_engine.py` because importing `infra.engine` transitively pulls in the vision / MLA / encoder-decoder model zoo and JambaEngine doesn't need any of it; `FLAEngine` handles the same problem the same way (its local `_ActiveSeq`).
...
| Model | TP | Scenario | Input/Output | vLLM (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-------------:|-------------:|------:|-----------------:|
| Jamba-tiny-dev | 1 | prefill-heavy | 1024/610  |  7,859 | 11,699 | **1.49x** | 436.4/610  |
| Jamba-tiny-dev | 1 | balanced      |  512/636  |  8,206 | 12,070 | **1.47x** | 349.5/636  |
| Jamba-tiny-dev | 1 | decode-heavy  |  512/1006 | 10,119 | 14,647 | **1.45x** | 773.1/1006 |

Geomean ratio: **1.47×** vs vLLM 0.18.0.  Avg match-tokens (tokens where kb-nano and vLLM agree exactly): **519/750** = 69.2% across the three scenarios.  With `KB_NANO_JAMBA_COMPILE=1` the geomean speedup rises to ~1.85× but match-tokens collapse (Inductor's bf16 reduction order diverges from vLLM's CUDA kernels) -- correctness wins by default.
...
| Model | TP | Scenario | Input/Output | vLLM (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-------------:|-------------:|------:|-----------------:|
| Jamba-v0.1 | 1 | prefill-heavy | 1024/618  | 1,152 | 1,165 | **1.01x** | 353.6/618 |
| Jamba-v0.1 | 1 | balanced      |  512/638  | 1,159 | 1,194 | **1.03x** | 290.5/638 |
| Jamba-v0.1 | 1 | decode-heavy  |  512/999  | 1,197 | 1,171 |  0.98x  | 583.3/999 |

Geomean ratio at 52B: **1.01×** vs vLLM (above the ≥0.9× ship bar with margin).  Avg match-tokens (kb-nano vs vLLM, exact-prefix-match): **409/752** = 54.3%, comfortably above the ≥50-token correctness bar in absolute count.  Reproducible across runs.  vLLM throughput shows ~5-15% per-scenario noise across runs depending on which other GPUs share the B200; kb-nano's absolute throughput is stable around 1,165-1,194 tok/s.
...
3. **Continuous batching with chunked prefill mid-decode.**  `Sequence` + `BlockManager` + `_MambaSlotPool` scheduler with `waiting / running` deques and three step types (mixed prefill+decode / pure prefill / pure decode); workspace sharing across `Attention` modules (`set_trtllm_workspace`, mirroring `LlamaEngine._share_trtllm_workspace`) reclaims ~3.5 GB on Blackwell.  The mixed-step path mirrors `_generate_mamba.run_mamba_mixed`'s prefill-first concatenated-batch convention; the L2 Mamba mixer has a native mixed-batch branch that splits at `num_prefill_tokens` and runs both `causal_conv1d_fn` + `selective_scan_fn` (prefill subset) and `causal_conv1d_update` + `selective_state_update` (decode subset) on the same global state slabs.
...
**Known limitation (documented for honesty):** within-seq prefill chunking (splitting one prompt's prefill across multiple steps when it exceeds `max_num_batched_tokens`) is not implemented.  `LlamaEngine` and `_generate_mamba` do this via per-seq `chunk_size` slicing inside `prepare_mixed_batch` / `_mamba_prepare_tensors`; the Jamba engine prefills each admitted prompt in one shot.  For typical prompts (≤16k tokens) and the standard bench above this is irrelevant; for users with very long prompts (>16k tokens) the prefill kernel would exceed scratch budget.  Workaround: lower `max_num_seqs` so the per-step total token count stays bounded.  Adding within-seq chunking is straightforward (the L2 Mamba mixer's `has_initial_state` flag already supports incremental state continuation; only the engine's `_run_prefill` and `_admit` need to track per-seq `num_computed_tokens` advancement) and is the next natural follow-up.
