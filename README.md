...
Run `tests/bench_jamba.py --model ai21labs/Jamba-tiny-dev` to reproduce. The reference is HuggingFace `JambaForCausalLM` driven via `.generate()` with HF's *fast* Mamba path enabled (`use_mamba_kernels=True`, the HF default). On this environment that path dispatches into the same `causal_conv1d_fn` / `causal_conv1d_update` / `selective_scan_fn` / `selective_state_update` CUDA primitives that kb-nano consumes as L1 ops (`causal-conv1d` 1.6.1 and `mamba-ssm` 2.3.1 are installed against torch 2.10 + CUDA 12.8 on the B200 in `/raid/user_data/olu/venv`). The benchmark is therefore a fair pipeline-vs-pipeline comparison: both sides use the same fused Mamba kernels, and the only difference is the surrounding orchestration. `--ref-slow-mamba` falls HF back to its Python-loop `slow_forward` path; that switch is for numerical / debugging use only and does **not** produce a fair perf number (it inflates kb-nano's speedup ~3–5× vs the fair fast-path baseline).

kb-nano's `JambaEngine` is intentionally simpler than the paged-KV `LlamaEngine` and the slot-based `MambaEngine` it conceptually unifies: it owns one transformer KV slab per batch (`[B, num_kv_heads, max_total, head_dim]`) and one Mamba `(conv_state, ssm_state)` slab per batch in a flat-varlen layout that we hand to vLLM's `causal_conv1d_fn` / `causal_conv1d_update` / `selective_scan_fn` / `selective_state_update` kernels directly. No paged KV cache, no chunked prefill, no continuous batching — micro-batches of size `max_num_seqs` flow through prefill → token-by-token decode in classic HF style. The throughput edge over HF here comes from running a 32-way micro-batch of full prefill+decode in our flat-varlen layout vs HF `.generate`'s 8-way micro-batch with a per-step Python driver — same kernels, different scheduling.

Throughput (Jamba-tiny-dev, 200 sequences per scenario, max-output=128, max-prompt=1024, `temperature=0`, kb-nano `max_num_seqs=32`, HF micro-batch=8):

| Model | TP | Scenario | Input/Output | HF (tok/s) | Ours (tok/s) | Ratio | Avg Match Tokens |
|-------|---:|----------|:------------:|-----------:|-------------:|------:|-----------------:|
| Jamba-tiny-dev | 1 | prefill-heavy | 1024/128 |   373 |  2,206 | **5.92x** | 101.5/128 |
