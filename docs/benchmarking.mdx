...
A scenario **passes** if its max error ratio is <= 1.0, where each element's error is normalized by `atol + rtol * abs(baseline)`. Speedup > 1.0 means faster than baseline.
