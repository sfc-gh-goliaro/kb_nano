...
All `fastkernels agent` runs are automatically logged to MLflow. You can also use the [tracking API](/experiment-tracking#tracking-api) in your own agent to log kernel generations, benchmark results, and custom metrics:
...
Use `fastkernels history` to query past runs from the CLI, or `fastkernels mlflow-ui` to launch the web UI. See [Experiment Tracking](/experiment-tracking) for full details.
