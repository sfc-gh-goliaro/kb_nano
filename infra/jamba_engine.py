...
class JambaEngine:
    ...
    def _run_batch(
        ...
        # ------------------------------------------------------------------
        # Allocate per-batch caches.  All zeros at start.
        # ------------------------------------------------------------------
        ...
        # Mamba: flat layout with uniform per-row stride T=max_prompt.
        # Each row is one "sequence" from the kernel's perspective.
        mamba_qsl_p = torch.tensor(
            [i * max_prompt for i in range(B + 1)],
            dtype=torch.int32, device=device,
        )
        mamba_has_init = torch.zeros(B, dtype=torch.bool, device=device)
        # Pad mask in the FLAT layout (row-major over [B, T]).
        ...
        # Decode-side attention mask: a single query (T_q=1) per row,
        # but the KV cache contains the (left-padded) prompt keys,
        # which include padded positions that the query MUST NOT attend
        # to.  Build a [B, 1, 1, max_total] mask with 0 for valid keys
        # and ``finfo.min`` for padded keys; we slice it to the live
        # KV length each step.
        decode_key_mask_full = torch.zeros(
            B, 1, 1, max_total, dtype=self.dtype, device=device,
        )
        # Padded prompt positions are masked.
        pad_positions = (attention_mask == 0)  # [B, max_prompt]
